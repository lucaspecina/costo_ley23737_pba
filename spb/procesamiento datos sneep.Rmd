...
# sneep <- read_csv("/Users/lucaspecina/Desktop/Data/Planificacion Estrategica/costo_ley23737_pba/datos complementarios/sneep-unificado-2002-2019.csv")
...
- NO23737_provs : delitos menos 23737, todas las provincias (seleccionadas)
- todos_2019 : todos los delitos de todas las provincias (no solo seleccionadas)
...
provinciales <- sneep %>% 
  ...
  mutate(DELITO = ifelse(delito1_descripcion=='Infracción ley n° 23.737 (estupefacientes)','delito_23737','delito_No_23737'))
...
# delitos por NO ley 23737 para las provincias
NO23737_provs <- provinciales %>%
  filter((delito1_descripcion!='Infracción ley n° 23.737 (estupefacientes)'))
          

todos_2019 <- sneep %>% filter(reparticion_descripcion=='Provincial') %>% filter(anio_censo==2019)
todos_2018 <- sneep %>% filter(reparticion_descripcion=='Provincial') %>% filter(anio_censo==2018)

```

#### Exploratorio de Provinciales (ver NAs y eso)
```{r}
nrow(provinciales %>% filter(anio_censo>=2018) %>% filter(provincia_descripcion == 'Buenos Aires'))

provinciales %>% filter(anio_censo>=2018) %>% filter(provincia_descripcion == 'Buenos Aires') %>%
  filter(is.na(delito1_descripcion)) %>% pull(delito1_descripcion) %>% length()

# esto nos dice que se puede trabajar bien incluso con la presencia de NAs porque no son muchos

```


#### Exploratorio - Tabla posiciones de delitos por provincias 2019 (todas no solo seleccionadas)
```{r}
# aca veo la distribucion de los delitos en delito1 cuando en delito2 es droga
todos_2019 %>% filter(delito2_descripcion=='Infracción ley n° 23.737 (estupefacientes)') %>%
  group_by(delito1_descripcion) %>% summarise(cantidad= n()) %>% arrange(desc(cantidad))

todos_2019 %>% 


posiciones_delitos_2019 <- todos_2019 %>% group_by(provincia_descripcion, delito1_descripcion) %>% summarise(cantidad= n()) %>% arrange(desc(provincia_descripcion, cantidad))


posiciones_delitos_2019 <- posiciones_delitos_2019 %>% group_by(provincia_descripcion) %>% mutate(porcentaje= cantidad/sum(cantidad)) %>% arrange(provincia_descripcion)

# reformar df long to wide -> spread / pivot_wider()
posiciones_delitos_2019 <- posiciones_delitos_2019 %>% pivot_wider(id_cols= delito1_descripcion, names_from=provincia_descripcion, values_from= porcentaje) %>% 
  mutate(across(everything(), ~replace_na(.x, 0)))

posiciones_delitos_2019
#write.xlsx(posiciones_delitos_2019, '/Users/lucaspecina/Desktop/Data/Planificacion Estrategica/costo_ley23737_pba/resultados complementarios/posiciones_delitos_2019.xlsx', na = "0")
```

#### Exploratorio - Tabla posiciones de delitos por provincias 2018 (todas no solo seleccionadas)

```{r}

posiciones_delitos_2018 <- todos_2018 %>% group_by(provincia_descripcion, delito1_descripcion) %>% summarise(cantidad= n()) %>% arrange(desc(provincia_descripcion, cantidad))


posiciones_delitos_2018 <- posiciones_delitos_2018 %>% group_by(provincia_descripcion) %>% mutate(porcentaje= cantidad/sum(cantidad)) %>% arrange(provincia_descripcion)

# reformar df long to wide -> spread / pivot_wider()
posiciones_delitos_2018 <- posiciones_delitos_2018 %>% pivot_wider(id_cols= delito1_descripcion, names_from=provincia_descripcion, values_from= porcentaje) %>% 
  mutate(across(everything(), ~replace_na(.x, 0)))

posiciones_delitos_2018
#write.xlsx(posiciones_delitos_2018, '/Users/lucaspecina/Desktop/Data/Planificacion Estrategica/costo_ley23737_pba/resultados complementarios/posiciones_delitos_2018.xlsx', na = "0")
```
...
Analisis para provincias seleccionadas pero solo 2019
```{r}
# cantidad de presos totales por año y provincia
presos_total_por_prov_19 <- provinciales %>% filter(anio_censo==2019) %>% group_by(provincia_descripcion) %>% 
  summarise(presos_totales= n())

# cantidad de presos droga por año y provincia
presos_ley23737_por_prov_19 <- ley23737_provs %>% filter(anio_censo==2019) %>% group_by(provincia_descripcion) %>%
  summarise(presos_ley23737= n())

# joineo
presos_porcen23737_por_prov_19 <- presos_total_por_prov_19 %>% 
  inner_join(presos_ley23737_por_prov_19, by=c('provincia_descripcion')) %>% 
  mutate(porcentaje_ley23737= presos_ley23737/presos_totales)

#write.xlsx(presos_porcen23737_por_prov_19,'/Users/lucaspecina/Desktop/Data/Planificacion Estrategica/costo_ley23737_pba/resultados complementarios/presos_porcen23737_2019.xlsx',row.names = FALSE)


```

Analisis para provincias seleccionadas pero solo 2018
```{r}
# cantidad de presos totales por año y provincia
presos_total_por_prov_18 <- provinciales %>% filter(anio_censo==2018) %>% group_by(provincia_descripcion) %>% 
  summarise(presos_totales= n())

# cantidad de presos droga por año y provincia
presos_ley23737_por_prov_18 <- ley23737_provs %>% filter(anio_censo==2018) %>% group_by(provincia_descripcion) %>%
  summarise(presos_ley23737= n())

# joineo
presos_porcen23737_por_prov_18 <- presos_total_por_prov_18 %>% 
  inner_join(presos_ley23737_por_prov_18, by=c('provincia_descripcion')) %>% 
  mutate(porcentaje_ley23737= presos_ley23737/presos_totales)

#write.xlsx(presos_porcen23737_por_prov_18,'/Users/lucaspecina/Desktop/Data/Planificacion Estrategica/costo_ley23737_pba/resultados complementarios/presos_porcen23737_2018.xlsx',row.names = FALSE)
...
Analisis para todas las provincias (DROGA Y NO DROGA)
---

Desde 2016
Buenos Aires, Chaco, Cordoba, Formosa, Salta, Entre Rios (18/19), Santiago del Estero (18/19), CABA (19)

#### Cantidad de presos NO DROGA y por droga, por año y provincia

```{r}
# agrupo todo junto (año, prov y delito (droga y no droga))
cantidad_presos_anio_prov <- provinciales %>% group_by(anio_censo,provincia_descripcion,DELITO) %>% 
  summarise(cantidad= n())

# spread para operar entre columnas
cantidad_presos_anio_prov <- cantidad_presos_anio_prov %>% spread(DELITO,cantidad)

# calculo porcentaje
cantidad_presos_anio_prov <- cantidad_presos_anio_prov %>% 
  mutate(porcen_ley23737 = (delito_23737/(delito_23737+delito_No_23737)))

# calculo cambios porcentuales por año
cantidad_presos_anio_prov <- cantidad_presos_anio_prov %>% group_by(provincia_descripcion) %>%
  mutate(cambioporc_no23737 = (delito_No_23737-lag(delito_No_23737))/lag(delito_No_23737) ,
         cambioporc_23737 = (delito_23737-lag(delito_23737))/lag(delito_23737))

# cambio porcentual acumulado
cantidad_presos_anio_prov <- cantidad_presos_anio_prov %>% group_by(provincia_descripcion) %>%
  mutate(cambioporc_no23737_acum = cumsum(replace_na(cambioporc_no23737,0)),
         cambioporc_23737_acum = cumsum(replace_na(cambioporc_23737,0)))

#write.xlsx(presos_porcen23737_por_anio_prov, '/Users/lucaspecina/Desktop/Data/Planificacion Estrategica/costo_ley23737_pba/resultados complementarios/cantidades_droga_nodroga.xlsx',row.names = FALSE)
# despues sigo en excel haciendo los cambios porcentuales, etc.

print(cantidad_presos_anio_prov)
```

#### Graficar por separado (facet) la evolucion del cambio porcentual de presos NO DROGA y por 23737
```{r fig20, fig.height = 10, fig.width = 8.5}
# acumulado
data_plot_pctch <- cantidad_presos_anio_prov %>%
  mutate(cambioporc_no23737_acum = cambioporc_no23737_acum*100, 
         cambioporc_23737_acum = cambioporc_23737_acum*100)
data_plot_pctch <- data_plot_pctch %>% 
  pivot_longer(c(cambioporc_no23737_acum,cambioporc_23737_acum), names_to='Tipo',values_to='Porcentaje') %>%
  mutate(Tipo= ifelse(Tipo=='cambioporc_no23737_acum','No Ley 23737','Ley 23737'))

ggplot(data_plot_pctch, aes(x=anio_censo,y=Porcentaje, color=Tipo)) + geom_line() + geom_point() +
    labs(title="Cambio porcentual acumulado de presxs según sean por ley 23737 o no ley 23737",x="Año de censo", y = "Cambio porcentual acumulado (%)") + facet_wrap(vars(provincia_descripcion), ncol=1, scales = "free_y")
...
Analisis para todas las provincias (DROGA Y TOTAL)
---
...
# cambio porcentual acumulado
presos_porcen23737_por_anio_prov <- presos_porcen23737_por_anio_prov %>% group_by(provincia_descripcion) %>%
  mutate(cambioporc_totales_acum = cumsum(replace_na(cambioporc_totales,0)),
         cambioporc_23737_acum = cumsum(replace_na(cambioporc_23737,0)))

#write.xlsx(presos_porcen23737_por_anio_prov, '/Users/lucaspecina/Desktop/Data/Planificacion Estrategica/costo_ley23737_pba/resultados complementarios/presos_porcen23737_por_anio_prov.xlsx',row.names = FALSE)
# despues sigo en excel haciendo los cambios porcentuales, etc.
...
#### Graficar por separado (facet)
```{r fig1, fig.height = 10, fig.width = 6}

ggplot(presos_porcen23737_por_anio_prov, aes(x=anio_censo,y=porcen_ley23737*100)) + geom_line() + geom_point() +
    labs(title="Porcentaje (%) de presos/as por ley 23737 por provincias elegidas",x="Año de censo", y = "Porcentaje de presos por 23737 (%)") + facet_wrap(vars(provincia_descripcion), ncol=1)


```

#### Graficar por separado (facet) la evolucion del cambio porcentual de presos totales y por 23737
```{r fig2, fig.height = 10, fig.width = 8}
# acumulado
data_plot_pctch <- presos_porcen23737_por_anio_prov %>%
  mutate(cambioporc_totales_acum = cambioporc_totales_acum*100, 
         cambioporc_23737_acum = cambioporc_23737_acum*100)
data_plot_pctch <- data_plot_pctch %>% 
  pivot_longer(c(cambioporc_totales_acum,cambioporc_23737_acum), names_to='Tipo',values_to='Porcentaje') %>%
  mutate(Tipo= ifelse(Tipo=='cambioporc_totales_acum','Todos delitos','Ley 23737'))

ggplot(data_plot_pctch, aes(x=anio_censo,y=Porcentaje, color=Tipo)) + geom_line() + geom_point() +
    labs(title="Cambio porcentual acumulado de presxs según sean por ley 23737 o total",x="Año de censo", y = "Cambio porcentual acumulado (%)") + facet_wrap(vars(provincia_descripcion), ncol=1, scales = "free_y")
...
POR GENERO (NO DROGA Y DROGA)

```{r}
# agrupo todo junto (año, prov y delito (droga y no droga))
cantidad_presos_anio_prov <- provinciales %>% group_by(anio_censo,provincia_descripcion,genero_descripcion,DELITO) %>% 
  summarise(cantidad= n())
...
# calculo porcentaje
cantidad_presos_anio_prov <- cantidad_presos_anio_prov %>% 
  mutate(genero_porcen_droga = (delito_23737/(delito_23737+delito_No_23737)))
...
#write.xlsx(presos_porcen23737_por_anio_prov, '/Users/lucaspecina/Desktop/Data/Planificacion Estrategica/costo_ley23737_pba/resultados complementarios/cantidades_droga_nodroga_genero.xlsx',row.names = FALSE)
# despues sigo en excel haciendo los cambios porcentuales, etc.
...
```{r}
data_plot_genero <- cantidad_presos_anio_prov %>%
  mutate(genero_porcen_droga = genero_porcen_droga*100)

ggplot(data_plot_genero, aes(x=anio_censo,y=genero_porcen_droga, color=genero_descripcion)) + geom_line() + geom_point() +
    labs(title="Porcentaje (%) de presos/as por ley 23737 por provincia y género",x="Año de censo", y = "Porcentaje de presos por 23737 (%)") + facet_wrap(vars(provincia_descripcion), ncol=2)
...
# calculo cambios porcentuales
presos_porcen23737_por_anio_prov <- presos_porcen23737_por_anio_prov %>% group_by(provincia_descripcion) %>%
  mutate(cambioporc_varones_total = (varones_total-lag(varones_total))/lag(varones_total),
         cambioporc_mujeres_total = (mujeres_total-lag(mujeres_total))/lag(mujeres_total),
         cambioporc_varones_23737 = (varones_droga-lag(varones_droga))/lag(varones_droga),
         cambioporc_mujeres_23737 = (mujeres_droga-lag(mujeres_droga))/lag(mujeres_droga))

# calculo cambios porcentuales acumulado
presos_porcen23737_por_anio_prov <- presos_porcen23737_por_anio_prov %>% group_by(provincia_descripcion) %>%
  mutate(cambioporc_acum_varones_total = cumsum(replace_na(cambioporc_varones_total,0)),
         cambioporc_acum_mujeres_total = cumsum(replace_na(cambioporc_mujeres_total,0)),
         cambioporc_acum_varones_23737 = cumsum(replace_na(cambioporc_varones_23737,0)),
         cambioporc_acum_mujeres_23737 = cumsum(replace_na(cambioporc_mujeres_23737,0)))



#write.xlsx(presos_porcen23737_por_anio_prov, '/Users/lucaspecina/Desktop/Data/Planificacion Estrategica/costo_ley23737_pba/resultados complementarios/presos_porcen23737_anio_provs_genero_cambio_acum.xlsx',row.names = FALSE)
...
Armar uno o un par de gráficos donde se observe con claridad: 

- El altísimo porcentaje de mujeres que están presas por estos delitos sobre el total de mujeres presas; en algunos casos, como por ejemplo, entre ríos es el 60%.
- Que el porcentaje de mujeres en delitos ley 23.737 es mayor al porcentaje general en la relación varones/mujeres privadxs de libertad; 

```{r}
data_plot_genero <- presos_porcen23737_por_anio_prov %>% filter(anio_censo==2019) %>% 
  mutate(porcen_ley23737 = porcen_ley23737*100, 
         mujeres_porcen_droga = mujeres_porcen_droga*100)
data_plot_genero <- data_plot_genero %>%
  pivot_longer(c(porcen_ley23737,mujeres_porcen_droga), names_to='tipo', values_to='cantidad')

ggplot(data_plot_genero, aes(x=tipo, y=cantidad, fill=tipo)) + geom_boxplot(notch=FALSE) + 
  geom_dotplot(binaxis='y', stackdir='center', dotsize=1) + 
  labs(title="Porcentajes de presos/as por ley 23737 según sean mujeres o general (2019)",x="Tipo (mujeres o presos/as general)", y = "Porcentaje de presos/as por ley 23.737 (%)") + theme_minimal()

###

data_plot_genero <- presos_porcen23737_por_anio_prov %>%
  mutate(porcen_ley23737 = porcen_ley23737*100, 
         mujeres_porcen_droga = mujeres_porcen_droga*100)
data_plot_genero <- data_plot_genero %>% 
  group_by(anio_censo) %>% summarise(mujeres_porcen_droga = mean(mujeres_porcen_droga),
                                     general_porcen_droga = mean(porcen_ley23737)) %>%
  pivot_longer(c(mujeres_porcen_droga,general_porcen_droga), names_to='Tipo',values_to='Porcentaje')

ggplot(data_plot_genero, aes(x=anio_censo, y=Porcentaje, color=Tipo)) + geom_line() + geom_point() +
  labs(title="Evolución temporal del porcentaje de presos/as por ley 23737 según sean mujeres o general",x="Año", y = "Porcentaje de presos/as por ley 23.737 (%)")  + theme_minimal() + theme(plot.title = element_text(size=11))
...
```{r}
data_plot_genero <- presos_porcen23737_por_anio_prov %>%
  mutate(varones_porcen_droga = varones_porcen_droga*100, 
         mujeres_porcen_droga = mujeres_porcen_droga*100)
data_plot_genero <- data_plot_genero %>% 
  pivot_longer(c(mujeres_porcen_droga,varones_porcen_droga), names_to='Tipo',values_to='Porcentaje') %>%
  mutate(Tipo= ifelse(Tipo=='mujeres_porcen_droga','Mujeres','Varones'))
...
nrow(provinciales_proce_conde %>% filter(anio_censo>=2018)) # 122652
provinciales_proce_conde %>% filter(anio_censo>=2018) %>%
  filter(is.na(delito1_descripcion)) %>% pull(delito1_descripcion) %>% length() # 1549
# la cantidad de NA en delito1 es muy poco, por lo tanto puedo trabajar bien
...
# hago un fix : agrego formosa 2015 y 2016 que quedan vacios porque no tienen ninguno. Les agrego 0 (rbind)
condenados_droga <- condenados_droga %>% add_row(anio_censo=2015,provincia_descripcion='Formosa',condenados_droga=0)
...
#write.xlsx(procesados_porcen_todos, '/Users/lucaspecina/Desktop/Data/Planificacion Estrategica/costo_ley23737_pba/resultados complementarios/proce_conde_porcen_todos.xlsx',row.names = FALSE)
```

#### Graficar por separado (facet) la evolucion de porcentaje de procesados por provs
```{r}
procesados_graficar <- read_excel("~/Desktop/Data/Planificacion Estrategica/costo_ley23737_pba/resultados complementarios/proce_conde_porcen_todos.xlsx")

data_plot_procesados <- procesados_graficar %>%
  mutate(procesados_porcen_droga = procesados_porcen_droga*100, 
         procesados_porcen_NOdroga = procesados_porcen_NOdroga*100)
data_plot_procesados <- data_plot_procesados %>% 
  pivot_longer(c(procesados_porcen_droga,procesados_porcen_NOdroga), names_to='Tipo',values_to='Porcentaje') %>%
  mutate(Tipo= ifelse(Tipo=='procesados_porcen_droga','Ley 23737','Todos delitos'))



ggplot(data_plot_procesados, aes(x=anio_censo,y=Porcentaje, color=Tipo)) + geom_line() + geom_point() +
    labs(title="Porcentaje (%) de presxs sin condena según sean por ley 23737 o total",x="Año de censo", y = "Porcentaje de presxs sin condena (%)") + facet_wrap(vars(provincia_descripcion), ncol=2)
...
ggplot(data_plotear_reinci, aes(x=anio_censo,y=porcentaje_reinci,color=tipo_delito)) + geom_line() + geom_point() +
    labs(title="Porcentajes de reincidentes entre drogas y no drogas (PBA)",x="Año de censo", y = "Porcentajes de reincidentes (%)") + theme_minimal()
...
ggplot(reinci_separado_droga, aes(x=anio_censo,y=cantidad,color=es_reincidente_descripcion)) + geom_line() + geom_point() + labs(title="Cantidad de presos/as por 23.737 segun reincidencia (PBA)",x="Año de censo", y = "Cantidad de presos/as") + theme_minimal()

```

#### plots reincidentes

Solo PBA 2019 ley 23737

```{r}

data_plot_nacionalidad_1 <- presos_porcen23737_por_anio_prov %>% filter(provincia_descripcion=='Buenos Aires') %>%
  pivot_longer(c(Argentina_droga,Paraguaya_droga,Peruana_droga,Boliviana_droga,Uruguaya_droga, Chilena_droga , Brasileña_droga, Colombiana_droga, Otras_droga), names_to='País', values_to='Cantidad') %>%
  mutate(País= str_remove(País, '_droga')) %>% select(c(País,Cantidad))

# library(stringr)
data_plot_nacionalidad_2 <- presos_porcen23737_por_anio_prov %>% filter(provincia_descripcion=='Buenos Aires') %>%
  pivot_longer(c("droga_porcen_Argentina" , "droga_porcen_Paraguaya" , "droga_porcen_Boliviana" , "droga_porcen_Peruana",  "droga_porcen_Uruguaya"  , "droga_porcen_Chilena"  ,  "droga_porcen_Brasileña" , "droga_porcen_Colombiana", "droga_porcen_Otras" ), names_to='País', values_to='Porcentaje') %>% mutate(Porcentaje=round(Porcentaje*100,1), País= str_remove(País, '^droga_porcen_')) %>% select(c(País,Porcentaje))

data_plot_nacionalidad <- data_plot_nacionalidad_1 %>% inner_join(data_plot_nacionalidad_2, by='País')


ggplot(data_plot_nacionalidad, aes(x=reorder(País,Porcentaje),y=Porcentaje, fill=Porcentaje)) + geom_bar(stat='identity') + 
  geom_text(aes(label=paste0(Porcentaje,'% (',Cantidad,')')), position=position_dodge(width=0.9), hjust=-0.1, vjust=0) + coord_flip() + 
  ylim(0.0,105) + labs(title="Nacionalidad de presos/as por ley 23.737 (PBA 2019)",x="Nacionalidades", y = "Porcentaje y cantidades") +
...
Solo PBA. Un gráfico donde se vea la información que esta a continuación. Diría que se vea sobre todo los porcentajes y entre paréntesis los números absolutos. 
```{r}
...
```{r}
colnames(presos_porcen23737_por_anio_prov)
```
...
presos_porcen23737_por_anio_prov <- presos_porcen23737_por_anio_prov %>% 
  mutate(
            total_porcen_Primario_incompleto = `Primario incompleto_total`/(`Primario incompleto_total`+ `Primario completo_total` + `Secundario incompleto_total` + `Secundario completo_total` + Ninguno_total + `Universitario completo_total` + `Universitario incompleto_total` + `Terciario completo_total` + `Terciario incompleto_total` + `Polimodal Incompleto_total` +`Polimodal Completo_total` +  `EGB 3 Completo_total` + `EGB 3 Incompleto_total` + `EGB 2 Completo_total` + `EGB 2 Incompleto_total` + `EGB 1 Completo_total` + `EGB 1 Incompleto_total`),
            total_porcen_Primario_completo = `Primario completo_total`/(`Primario incompleto_total`+ `Primario completo_total` + `Secundario incompleto_total` + `Secundario completo_total` + Ninguno_total + `Universitario completo_total` + `Universitario incompleto_total` + `Terciario completo_total` + `Terciario incompleto_total` + `Polimodal Incompleto_total` +`Polimodal Completo_total` +  `EGB 3 Completo_total` + `EGB 3 Incompleto_total` + `EGB 2 Completo_total` + `EGB 2 Incompleto_total` + `EGB 1 Completo_total` + `EGB 1 Incompleto_total`),
            total_porcen_Secundario_incompleto = `Secundario incompleto_total`/(`Primario incompleto_total`+ `Primario completo_total` + `Secundario incompleto_total` + `Secundario completo_total` + Ninguno_total + `Universitario completo_total` + `Universitario incompleto_total` + `Terciario completo_total` + `Terciario incompleto_total` + `Polimodal Incompleto_total` +`Polimodal Completo_total` +  `EGB 3 Completo_total` + `EGB 3 Incompleto_total` + `EGB 2 Completo_total` + `EGB 2 Incompleto_total` + `EGB 1 Completo_total` + `EGB 1 Incompleto_total`),
            total_porcen_Secundario_completo = `Secundario completo_total`/(`Primario incompleto_total`+ `Primario completo_total` + `Secundario incompleto_total` + `Secundario completo_total` + Ninguno_total + `Universitario completo_total` + `Universitario incompleto_total` + `Terciario completo_total` + `Terciario incompleto_total` + `Polimodal Incompleto_total` +`Polimodal Completo_total` +  `EGB 3 Completo_total` + `EGB 3 Incompleto_total` + `EGB 2 Completo_total` + `EGB 2 Incompleto_total` + `EGB 1 Completo_total` + `EGB 1 Incompleto_total`),
            total_porcen_Ninguno = Ninguno_total/(`Primario incompleto_total`+ `Primario completo_total` + `Secundario incompleto_total` + `Secundario completo_total` + Ninguno_total + `Universitario completo_total` + `Universitario incompleto_total` + `Terciario completo_total` + `Terciario incompleto_total` + `Polimodal Incompleto_total` +`Polimodal Completo_total` +  `EGB 3 Completo_total` + `EGB 3 Incompleto_total` + `EGB 2 Completo_total` + `EGB 2 Incompleto_total` + `EGB 1 Completo_total` + `EGB 1 Incompleto_total`),
            total_porcen_Universitario_completo = `Universitario completo_total`/(`Primario incompleto_total`+ `Primario completo_total` + `Secundario incompleto_total` + `Secundario completo_total` + Ninguno_total + `Universitario completo_total` + `Universitario incompleto_total` + `Terciario completo_total` + `Terciario incompleto_total` + `Polimodal Incompleto_total` +`Polimodal Completo_total` +  `EGB 3 Completo_total` + `EGB 3 Incompleto_total` + `EGB 2 Completo_total` + `EGB 2 Incompleto_total` + `EGB 1 Completo_total` + `EGB 1 Incompleto_total`),
            total_porcen_Universitario_incompleto = `Universitario incompleto_total`/(`Primario incompleto_total`+ `Primario completo_total` + `Secundario incompleto_total` + `Secundario completo_total` + Ninguno_total + `Universitario completo_total` + `Universitario incompleto_total` + `Terciario completo_total` + `Terciario incompleto_total` + `Polimodal Incompleto_total` +`Polimodal Completo_total` +  `EGB 3 Completo_total` + `EGB 3 Incompleto_total` + `EGB 2 Completo_total` + `EGB 2 Incompleto_total` + `EGB 1 Completo_total` + `EGB 1 Incompleto_total`),
            total_porcen_Terciario_completo = `Terciario completo_total`/(`Primario incompleto_total`+ `Primario completo_total` + `Secundario incompleto_total` + `Secundario completo_total` + Ninguno_total + `Universitario completo_total` + `Universitario incompleto_total` + `Terciario completo_total` + `Terciario incompleto_total` + `Polimodal Incompleto_total` +`Polimodal Completo_total` +  `EGB 3 Completo_total` + `EGB 3 Incompleto_total` + `EGB 2 Completo_total` + `EGB 2 Incompleto_total` + `EGB 1 Completo_total` + `EGB 1 Incompleto_total`),
            total_porcen_Terciario_incompleto = `Terciario incompleto_total`/(`Primario incompleto_total`+ `Primario completo_total` + `Secundario incompleto_total` + `Secundario completo_total` + Ninguno_total + `Universitario completo_total` + `Universitario incompleto_total` + `Terciario completo_total` + `Terciario incompleto_total` + `Polimodal Incompleto_total` +`Polimodal Completo_total` +  `EGB 3 Completo_total` + `EGB 3 Incompleto_total` + `EGB 2 Completo_total` + `EGB 2 Incompleto_total` + `EGB 1 Completo_total` + `EGB 1 Incompleto_total`),
            total_porcen_Polimodal_Incompleto = `Polimodal Incompleto_total`/(`Primario incompleto_total`+ `Primario completo_total` + `Secundario incompleto_total` + `Secundario completo_total` + Ninguno_total + `Universitario completo_total` + `Universitario incompleto_total` + `Terciario completo_total` + `Terciario incompleto_total` + `Polimodal Incompleto_total` +`Polimodal Completo_total` +  `EGB 3 Completo_total` + `EGB 3 Incompleto_total` + `EGB 2 Completo_total` + `EGB 2 Incompleto_total` + `EGB 1 Completo_total` + `EGB 1 Incompleto_total`),
            total_porcen_Polimodal_Completo = `Polimodal Completo_total`/(`Primario incompleto_total`+ `Primario completo_total` + `Secundario incompleto_total` + `Secundario completo_total` + Ninguno_total + `Universitario completo_total` + `Universitario incompleto_total` + `Terciario completo_total` + `Terciario incompleto_total` + `Polimodal Incompleto_total` +`Polimodal Completo_total` +  `EGB 3 Completo_total` + `EGB 3 Incompleto_total` + `EGB 2 Completo_total` + `EGB 2 Incompleto_total` + `EGB 1 Completo_total` + `EGB 1 Incompleto_total`),
            total_porcen_EGB_3_Completo = `EGB 3 Completo_total`/(`Primario incompleto_total`+ `Primario completo_total` + `Secundario incompleto_total` + `Secundario completo_total` + Ninguno_total + `Universitario completo_total` + `Universitario incompleto_total` + `Terciario completo_total` + `Terciario incompleto_total` + `Polimodal Incompleto_total` +`Polimodal Completo_total` +  `EGB 3 Completo_total` + `EGB 3 Incompleto_total` + `EGB 2 Completo_total` + `EGB 2 Incompleto_total` + `EGB 1 Completo_total` + `EGB 1 Incompleto_total`),
            total_porcen_EGB_3_Incompleto = `EGB 3 Incompleto_total`/(`Primario incompleto_total`+ `Primario completo_total` + `Secundario incompleto_total` + `Secundario completo_total` + Ninguno_total + `Universitario completo_total` + `Universitario incompleto_total` + `Terciario completo_total` + `Terciario incompleto_total` + `Polimodal Incompleto_total` +`Polimodal Completo_total` +  `EGB 3 Completo_total` + `EGB 3 Incompleto_total` + `EGB 2 Completo_total` + `EGB 2 Incompleto_total` + `EGB 1 Completo_total` + `EGB 1 Incompleto_total`),
            total_porcen_EGB_2_Completo = `EGB 2 Completo_total`/(`Primario incompleto_total`+ `Primario completo_total` + `Secundario incompleto_total` + `Secundario completo_total` + Ninguno_total + `Universitario completo_total` + `Universitario incompleto_total` + `Terciario completo_total` + `Terciario incompleto_total` + `Polimodal Incompleto_total` +`Polimodal Completo_total` +  `EGB 3 Completo_total` + `EGB 3 Incompleto_total` + `EGB 2 Completo_total` + `EGB 2 Incompleto_total` + `EGB 1 Completo_total` + `EGB 1 Incompleto_total`),
            total_porcen_EGB_2_Incompleto = `EGB 2 Incompleto_total`/(`Primario incompleto_total`+ `Primario completo_total` + `Secundario incompleto_total` + `Secundario completo_total` + Ninguno_total + `Universitario completo_total` + `Universitario incompleto_total` + `Terciario completo_total` + `Terciario incompleto_total` + `Polimodal Incompleto_total` +`Polimodal Completo_total` +  `EGB 3 Completo_total` + `EGB 3 Incompleto_total` + `EGB 2 Completo_total` + `EGB 2 Incompleto_total` + `EGB 1 Completo_total` + `EGB 1 Incompleto_total`),
            total_porcen_EGB_1_Completo = `EGB 1 Completo_total`/(`Primario incompleto_total`+ `Primario completo_total` + `Secundario incompleto_total` + `Secundario completo_total` + Ninguno_total + `Universitario completo_total` + `Universitario incompleto_total` + `Terciario completo_total` + `Terciario incompleto_total` + `Polimodal Incompleto_total` +`Polimodal Completo_total` +  `EGB 3 Completo_total` + `EGB 3 Incompleto_total` + `EGB 2 Completo_total` + `EGB 2 Incompleto_total` + `EGB 1 Completo_total` + `EGB 1 Incompleto_total`),
...
presos_porcen23737_por_anio_prov <- presos_porcen23737_por_anio_prov %>% 
  mutate(
            droga_porcen_Primario_incompleto = `Primario incompleto_droga`/(`Primario incompleto_droga`+ `Primario completo_droga` + `Secundario incompleto_droga` + `Secundario completo_droga` + Ninguno_droga + `Universitario completo_droga` + `Universitario incompleto_droga` + `Terciario completo_droga` + `Terciario incompleto_droga` + `Polimodal Incompleto_droga` +`Polimodal Completo_droga` +  `EGB 3 Completo_droga` + `EGB 3 Incompleto_droga` + `EGB 2 Completo_droga` + `EGB 2 Incompleto_droga` + `EGB 1 Completo_droga` + `EGB 1 Incompleto_droga`),
            droga_porcen_Primario_completo = `Primario completo_droga`/(`Primario incompleto_droga`+ `Primario completo_droga` + `Secundario incompleto_droga` + `Secundario completo_droga` + Ninguno_droga + `Universitario completo_droga` + `Universitario incompleto_droga` + `Terciario completo_droga` + `Terciario incompleto_droga` + `Polimodal Incompleto_droga` +`Polimodal Completo_droga` +  `EGB 3 Completo_droga` + `EGB 3 Incompleto_droga` + `EGB 2 Completo_droga` + `EGB 2 Incompleto_droga` + `EGB 1 Completo_droga` + `EGB 1 Incompleto_droga`),
            droga_porcen_Secundario_incompleto = `Secundario incompleto_droga`/(`Primario incompleto_droga`+ `Primario completo_droga` + `Secundario incompleto_droga` + `Secundario completo_droga` + Ninguno_droga + `Universitario completo_droga` + `Universitario incompleto_droga` + `Terciario completo_droga` + `Terciario incompleto_droga` + `Polimodal Incompleto_droga` +`Polimodal Completo_droga` +  `EGB 3 Completo_droga` + `EGB 3 Incompleto_droga` + `EGB 2 Completo_droga` + `EGB 2 Incompleto_droga` + `EGB 1 Completo_droga` + `EGB 1 Incompleto_droga`),
            droga_porcen_Secundario_completo = `Secundario completo_droga`/(`Primario incompleto_droga`+ `Primario completo_droga` + `Secundario incompleto_droga` + `Secundario completo_droga` + Ninguno_droga + `Universitario completo_droga` + `Universitario incompleto_droga` + `Terciario completo_droga` + `Terciario incompleto_droga` + `Polimodal Incompleto_droga` +`Polimodal Completo_droga` +  `EGB 3 Completo_droga` + `EGB 3 Incompleto_droga` + `EGB 2 Completo_droga` + `EGB 2 Incompleto_droga` + `EGB 1 Completo_droga` + `EGB 1 Incompleto_droga`),
            droga_porcen_Ninguno = Ninguno_droga/(`Primario incompleto_droga`+ `Primario completo_droga` + `Secundario incompleto_droga` + `Secundario completo_droga` + Ninguno_droga + `Universitario completo_droga` + `Universitario incompleto_droga` + `Terciario completo_droga` + `Terciario incompleto_droga` + `Polimodal Incompleto_droga` +`Polimodal Completo_droga` +  `EGB 3 Completo_droga` + `EGB 3 Incompleto_droga` + `EGB 2 Completo_droga` + `EGB 2 Incompleto_droga` + `EGB 1 Completo_droga` + `EGB 1 Incompleto_droga`),
            droga_porcen_Universitario_completo = `Universitario completo_droga`/(`Primario incompleto_droga`+ `Primario completo_droga` + `Secundario incompleto_droga` + `Secundario completo_droga` + Ninguno_droga + `Universitario completo_droga` + `Universitario incompleto_droga` + `Terciario completo_droga` + `Terciario incompleto_droga` + `Polimodal Incompleto_droga` +`Polimodal Completo_droga` +  `EGB 3 Completo_droga` + `EGB 3 Incompleto_droga` + `EGB 2 Completo_droga` + `EGB 2 Incompleto_droga` + `EGB 1 Completo_droga` + `EGB 1 Incompleto_droga`),
            droga_porcen_Universitario_incompleto = `Universitario incompleto_droga`/(`Primario incompleto_droga`+ `Primario completo_droga` + `Secundario incompleto_droga` + `Secundario completo_droga` + Ninguno_droga + `Universitario completo_droga` + `Universitario incompleto_droga` + `Terciario completo_droga` + `Terciario incompleto_droga` + `Polimodal Incompleto_droga` +`Polimodal Completo_droga` +  `EGB 3 Completo_droga` + `EGB 3 Incompleto_droga` + `EGB 2 Completo_droga` + `EGB 2 Incompleto_droga` + `EGB 1 Completo_droga` + `EGB 1 Incompleto_droga`),
            droga_porcen_Terciario_completo = `Terciario completo_droga`/(`Primario incompleto_droga`+ `Primario completo_droga` + `Secundario incompleto_droga` + `Secundario completo_droga` + Ninguno_droga + `Universitario completo_droga` + `Universitario incompleto_droga` + `Terciario completo_droga` + `Terciario incompleto_droga` + `Polimodal Incompleto_droga` +`Polimodal Completo_droga` +  `EGB 3 Completo_droga` + `EGB 3 Incompleto_droga` + `EGB 2 Completo_droga` + `EGB 2 Incompleto_droga` + `EGB 1 Completo_droga` + `EGB 1 Incompleto_droga`),
            droga_porcen_Terciario_incompleto = `Terciario incompleto_droga`/(`Primario incompleto_droga`+ `Primario completo_droga` + `Secundario incompleto_droga` + `Secundario completo_droga` + Ninguno_droga + `Universitario completo_droga` + `Universitario incompleto_droga` + `Terciario completo_droga` + `Terciario incompleto_droga` + `Polimodal Incompleto_droga` +`Polimodal Completo_droga` +  `EGB 3 Completo_droga` + `EGB 3 Incompleto_droga` + `EGB 2 Completo_droga` + `EGB 2 Incompleto_droga` + `EGB 1 Completo_droga` + `EGB 1 Incompleto_droga`),
            droga_porcen_Polimodal_Incompleto = `Polimodal Incompleto_droga`/(`Primario incompleto_droga`+ `Primario completo_droga` + `Secundario incompleto_droga` + `Secundario completo_droga` + Ninguno_droga + `Universitario completo_droga` + `Universitario incompleto_droga` + `Terciario completo_droga` + `Terciario incompleto_droga` + `Polimodal Incompleto_droga` +`Polimodal Completo_droga` +  `EGB 3 Completo_droga` + `EGB 3 Incompleto_droga` + `EGB 2 Completo_droga` + `EGB 2 Incompleto_droga` + `EGB 1 Completo_droga` + `EGB 1 Incompleto_droga`),
            droga_porcen_Polimodal_Completo = `Polimodal Completo_droga`/(`Primario incompleto_droga`+ `Primario completo_droga` + `Secundario incompleto_droga` + `Secundario completo_droga` + Ninguno_droga + `Universitario completo_droga` + `Universitario incompleto_droga` + `Terciario completo_droga` + `Terciario incompleto_droga` + `Polimodal Incompleto_droga` +`Polimodal Completo_droga` +  `EGB 3 Completo_droga` + `EGB 3 Incompleto_droga` + `EGB 2 Completo_droga` + `EGB 2 Incompleto_droga` + `EGB 1 Completo_droga` + `EGB 1 Incompleto_droga`),
            droga_porcen_EGB_3_Completo = `EGB 3 Completo_droga`/(`Primario incompleto_droga`+ `Primario completo_droga` + `Secundario incompleto_droga` + `Secundario completo_droga` + Ninguno_droga + `Universitario completo_droga` + `Universitario incompleto_droga` + `Terciario completo_droga` + `Terciario incompleto_droga` + `Polimodal Incompleto_droga` +`Polimodal Completo_droga` +  `EGB 3 Completo_droga` + `EGB 3 Incompleto_droga` + `EGB 2 Completo_droga` + `EGB 2 Incompleto_droga` + `EGB 1 Completo_droga` + `EGB 1 Incompleto_droga`),
            droga_porcen_EGB_3_Incompleto = `EGB 3 Incompleto_droga`/(`Primario incompleto_droga`+ `Primario completo_droga` + `Secundario incompleto_droga` + `Secundario completo_droga` + Ninguno_droga + `Universitario completo_droga` + `Universitario incompleto_droga` + `Terciario completo_droga` + `Terciario incompleto_droga` + `Polimodal Incompleto_droga` +`Polimodal Completo_droga` +  `EGB 3 Completo_droga` + `EGB 3 Incompleto_droga` + `EGB 2 Completo_droga` + `EGB 2 Incompleto_droga` + `EGB 1 Completo_droga` + `EGB 1 Incompleto_droga`),
            droga_porcen_EGB_2_Completo = `EGB 2 Completo_droga`/(`Primario incompleto_droga`+ `Primario completo_droga` + `Secundario incompleto_droga` + `Secundario completo_droga` + Ninguno_droga + `Universitario completo_droga` + `Universitario incompleto_droga` + `Terciario completo_droga` + `Terciario incompleto_droga` + `Polimodal Incompleto_droga` +`Polimodal Completo_droga` +  `EGB 3 Completo_droga` + `EGB 3 Incompleto_droga` + `EGB 2 Completo_droga` + `EGB 2 Incompleto_droga` + `EGB 1 Completo_droga` + `EGB 1 Incompleto_droga`),
            droga_porcen_EGB_2_Incompleto = `EGB 2 Incompleto_droga`/(`Primario incompleto_droga`+ `Primario completo_droga` + `Secundario incompleto_droga` + `Secundario completo_droga` + Ninguno_droga + `Universitario completo_droga` + `Universitario incompleto_droga` + `Terciario completo_droga` + `Terciario incompleto_droga` + `Polimodal Incompleto_droga` +`Polimodal Completo_droga` +  `EGB 3 Completo_droga` + `EGB 3 Incompleto_droga` + `EGB 2 Completo_droga` + `EGB 2 Incompleto_droga` + `EGB 1 Completo_droga` + `EGB 1 Incompleto_droga`),
            droga_porcen_EGB_1_Completo = `EGB 1 Completo_droga`/(`Primario incompleto_droga`+ `Primario completo_droga` + `Secundario incompleto_droga` + `Secundario completo_droga` + Ninguno_droga + `Universitario completo_droga` + `Universitario incompleto_droga` + `Terciario completo_droga` + `Terciario incompleto_droga` + `Polimodal Incompleto_droga` +`Polimodal Completo_droga` +  `EGB 3 Completo_droga` + `EGB 3 Incompleto_droga` + `EGB 2 Completo_droga` + `EGB 2 Incompleto_droga` + `EGB 1 Completo_droga` + `EGB 1 Incompleto_droga`),
            droga_porcen_EGB_1_Incompleto = `EGB 1 Incompleto_droga`/(`Primario incompleto_droga`+ `Primario completo_droga` + `Secundario incompleto_droga` + `Secundario completo_droga` + Ninguno_droga + `Universitario completo_droga` + `Universitario incompleto_droga` + `Terciario completo_droga` + `Terciario incompleto_droga` + `Polimodal Incompleto_droga` +`Polimodal Completo_droga` +  `EGB 3 Completo_droga` + `EGB 3 Incompleto_droga` + `EGB 2 Completo_droga` + `EGB 2 Incompleto_droga` + `EGB 1 Completo_droga` + `EGB 1 Incompleto_droga`)  )
...
#### plots nivel de instruccion

Solo PBA. Solo 2019. Solo ley 23.737. Un grafico con la siguiente información donde se vean bien los porcentajes y entre paréntesis los números absolutos. Las categorías que representan menos de un uno por ciento agrúpalas en un “otros”.
```{r}

data_plot_instruccion_1 <- presos_porcen23737_por_anio_prov %>% filter(anio_censo==2019) %>%
  pivot_longer(c("Primario incompleto_droga", "Primario completo_droga", "Secundario incompleto_droga", 'Ninguno_droga', "Universitario completo_droga", "Secundario completo_droga" , "Terciario completo_droga" , "Terciario incompleto_droga",'Universitario incompleto_droga'), names_to='Nivel_instruccion', values_to='Cantidad') %>%
  mutate(Nivel_instruccion= str_remove(Nivel_instruccion, '_droga')) %>% select(c(Nivel_instruccion,Cantidad)) 

# library(stringr)
data_plot_instruccion_2 <- presos_porcen23737_por_anio_prov %>% filter(anio_censo==2019) %>%
  pivot_longer(c("droga_porcen_Primario_incompleto", "droga_porcen_Primario_completo", "droga_porcen_Secundario_incompleto"   , "droga_porcen_Secundario_completo", "droga_porcen_Ninguno", "droga_porcen_Universitario_completo", "droga_porcen_Universitario_incompleto", "droga_porcen_Terciario_completo", "droga_porcen_Terciario_incompleto", "droga_porcen_Polimodal_Incompleto", "droga_porcen_Polimodal_Completo"), names_to='Nivel_instruccion', values_to='Porcentaje') %>% mutate(Porcentaje=round(Porcentaje*100,1), Nivel_instruccion= str_remove(Nivel_instruccion, '^droga_porcen_')) %>% select(c(Nivel_instruccion,Porcentaje)) %>% mutate(Nivel_instruccion = str_replace(Nivel_instruccion, '_',' '))

data_plot_instruccion <- data_plot_instruccion_1 %>% inner_join(data_plot_instruccion_2, by='Nivel_instruccion')


ggplot(data_plot_instruccion, aes(x=reorder(Nivel_instruccion,Porcentaje),y=Porcentaje, fill=Porcentaje)) + geom_bar(stat='identity') + 
  geom_text(aes(label=paste0(Porcentaje,'% (',Cantidad,')')), position=position_dodge(width=0.9), hjust=-0.1, vjust=0) + coord_flip() + 
  ylim(0.0,50) + labs(title="Nivel de instrucción de presos/as por ley 23.737 (PBA 2019)",x="Nivel de instrucción", y = "Porcentaje y cantidades") + theme_minimal() +
  theme(legend.position = "none") 
...
#### plots situacion laboral

Solo PBA 2019 LEY 23.737

```{r}

data_plot_situacionlaboral_1 <- presos_porcen23737_por_anio_prov %>% filter(anio_censo==2019) %>%
  pivot_longer(c("Desocupado_droga", "Trabajador de tiempo completo_droga", "Trabajador de tiempo parcial_droga"), names_to='Situacion_laboral', values_to='Cantidad') %>%
  mutate(Situacion_laboral= str_remove(Situacion_laboral, '_droga')) %>% select(c(Situacion_laboral,Cantidad)) 

# library(stringr)
data_plot_situacionlaboral_2 <- presos_porcen23737_por_anio_prov %>% filter(anio_censo==2019) %>%
  pivot_longer(c("droga_porcen_Desocupado", "droga_porcen_Trabajador_de_tiempo_completo", "droga_porcen_Trabajador_de_tiempo_parcial" ), names_to='Situacion_laboral', values_to='Porcentaje') %>% mutate(Porcentaje=round(Porcentaje*100,1), Situacion_laboral= str_remove(Situacion_laboral, '^droga_porcen_')) %>% select(c(Situacion_laboral,Porcentaje)) %>% mutate(Situacion_laboral = str_replace_all(Situacion_laboral, '_',' '))

data_plot_situacionlaboral <- data_plot_situacionlaboral_1 %>% inner_join(data_plot_situacionlaboral_2, by='Situacion_laboral')


ggplot(data_plot_situacionlaboral, aes(x=reorder(Situacion_laboral,Porcentaje),y=Porcentaje, fill=Porcentaje)) + geom_bar(stat='identity') + 
  geom_text(aes(label=paste0(Porcentaje,'% (',Cantidad,')')), position=position_dodge(width=0.9), hjust=-0.1, vjust=0) + coord_flip() + 
  ylim(0.0,60) + labs(title="Última situación laboral de presos/as por ley 23.737 (PBA 2019)",x="Situación laboral", y = "Porcentaje y cantidades") +
  theme(legend.position = "none") 
